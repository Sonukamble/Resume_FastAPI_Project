...
from resume_filter_API.database.DatabaseConnection import get_db
from resume_filter_API.entity.PydanticModel import UserCreate
from resume_filter_API.entity.models import Users, UserFile, UserQuestions
from resume_filter_API.operation.JwtTokenCreation import verify_password, verify_password_reset_token
...
def verify_password_reset_token_and_get_email(access_toekn: str):
    try:
        verified_username = verify_password_reset_token(access_toekn)

        # Manually retrieve the session from the generator
        db = next(get_db())
        # Query the user by email
        user = db.query(Users).filter(Users.username == verified_username).first()
